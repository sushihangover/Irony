...
namespace Medsphere.Widgets
{
	...
	public class CellPressEventArgs : ButtonPressEventArgs
	{
		public int Row;
		public int Column;
		public TreePath Path;
	}
	public delegate void CellPressEventHandler(object sender, CellPressEventArgs e);
	public class GridView : Container
	{
		public event CellPressEventHandler CellPressEvent;
		...
		private void OnFDAButtonPressEvent (object o, ButtonPressEventArgs a)
		{
			...
			{
				selection.Deselect ();
				//Debug.Print ("Cell; Col:{0}; Row: {1} deselect", x - c_span, y - r_span);

			} else {
				selection.SelectCell (path, col);
				ScrollToCell (path, col, false, 0, 0);
				//Debug.Print ("Cell; Col:{0}; Row: {1} select", x - c_span, y - r_span);
				var foo = new CellPressEventArgs ();
				foo.Column = x - c_span;
				foo.Row = y - r_span;
				foo.Path = path;
				foo.Args = a.Args;
				if ( CellPressEvent != null )
					CellPressEvent(o, foo);
			}
		}
